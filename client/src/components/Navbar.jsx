...
const Navbar = () => {
    ...
    const {axios, user, setUser, setShowUserLogin,navigate,searchQuery, setSearchQuery,getCartCount} = useAppContext();
    const logout  = async () => {
        try {
    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                {!user ? (<button onClick={()=>{setShowUserLogin(true);}} className="cursor-pointer px-8 py-2 bg-primary hbg-primary-dull transition text-white rounded-full">
                    Login
                </button>):
                (
            ...
            <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden z-50`}>
                ...
                <NavLink to = '/contact' onClick={()=>{setOpen(false)}}>Contact Us</NavLink>
                {
                    !user?(
                        ...
                        }} className="cursor-pointer px-6 py-2 mt-2 bg-primary hbg-primary-dull transition text-white rounded-full text-sm">
                            Login
                        </button>
                    ):(
                        <button onClick={
